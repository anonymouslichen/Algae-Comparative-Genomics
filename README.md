...
The translated orthologs of the SOG (`data/hhpred_query.fasta`, seven taxa) were
aligned into a single query profile and searched against the PDB70, Pfam, SMART,
and SCOPe70 profile-HMM databases with default parameters (accessed 2026-07-08).
The full search output is provided as `data/hhpred_result.hhr`.
...
Intermediate tables output by the pipeline and used in the R scripts.
...
| `hhpred_query.fasta` | Translated orthologs of `OG0003934` (seven taxa) used as the HHpred query profile |
| `hhpred_result.hhr` | HHpred structural-homology search output for `OG0003934` (identifies lysyl-tRNA ligase) |
...
this repository, then reviewed, edited, and verified for accuracy by A. R. Meyer.*
